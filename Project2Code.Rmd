...
dayData<-channelData%>%select(shares,starts_with("weekday_is"))%>%
  ...
  group_by(day_of_week)%>%
    dplyr::summarize(avg_Shares=mean(shares, na.rm=TRUE))
...
#Build model with 20 highest (pairwise) correlated  variables to shares

#save the names of the 20 predictors with highest correlation as a row vector in order to  use later for a linear model
variables<- unname(unlist(slice(share_cor,1:15)[1]))
RHSformu<-paste(variables, collapse="+")
formu<-as.formula(paste("shares~",RHSformu))
...
resamps <- caret::resamples(list(lmFit1,
                          lmFit2,
                           rfFit,
                          btFit))
summary(resamps)
