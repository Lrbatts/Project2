...
In order to explore the number of shares we are dealing with, a 5 number summary shows the distribution, along with standard deviation to get an idea of the spread
```{r number summary and boxplot}
...
We can again inspect the shares variable by using an empirical cumulative distribution function. The graph displays the amount of shares with the proportion of points at or below the value. As the line approaches 1, more of th edata is represented by that value of shares and lesser to give us an idea of the where the majority of shares per article lies at and below.
...
``` {r linear regression model 1, cache=TRUE}
...
```{r linear regression model 2, cache=TRUE}
